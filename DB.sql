...
CREATE TABLE IF NOT EXISTS `courses`.`student_has_course` (
  ...
  INDEX `student_course_ibfk_1_idx` (`course_id` ASC) VISIBLE,
  INDEX `student_course_ibfk_2_idx` (`user_id` ASC) VISIBLE,
  CONSTRAINT `student_course_ibfk_1`
    FOREIGN KEY (`course_id`)
    REFERENCES `courses`.`course` (`course_id`)
    ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS `courses`.`Notas` (
  ...
  `nota` INT NOT NULL,
  `identify_note` TINYINT(1) NOT NULL CHECK (identify_note BETWEEN 1 AND 9),
...
INSERT INTO `courses`.`user`(`user_id`,`first_name`,`last_name`,`email`,`username`,`password`) VALUES (1,'Juan','Pérez','juan.perez@example.com','juan.perez','$2b$04$pshU8fGJxhK1nZ8/jz8x3OvP7jBspj0L9YRm9jQVWVghri/.Nu1IG')
